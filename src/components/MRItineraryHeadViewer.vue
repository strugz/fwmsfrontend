                ...
                <v-layout row wrap>
                    ...
                    <v-flex sm4 md4 lg6 xs12>
                        <v-combobox outline v-model="selectedItemUser" :items="userList" item-text="CNTMNN"
                            item-value="CNTMID" label="Select User" @change="dataReload"></v-combobox>
                    </v-flex>
                </v-layout>
                <v-flex sm4 xs12>
                    ...
                    <v-calendar ref="calendar" v-model="today" type="month" color="primary">
                        <template v-slot:day="{ date }">
                            <template v-for="event in eventsMap[date]">
                                <v-menu :key="event.itimid" v-model="event.open" :close-on-content-click="false"
                                    full-width offset-x>
                                    <template v-slot:activator="{ on }">
                                        ...
                                        <div v-else>
                                            <div v-show="event.trdsts != 'LEAVE'">
                                                <div :class="event.trdsts == 'START' ? 'my-event' : event.trdsts == 'WORK COMPLETE' ? 'my-event1' : 'my-event2'"
                                                    v-if="!event.time" v-ripple v-on="on">
                                                    <span v-if="event.trdsts == 'LEAVE'">{{ event.title }}</span>
                                                    <span v-if="event.trdsts != 'LEAVE'">{{ event.client }}</span>
                                                    <span v-show="event.trdsts == 'WORK COMPLETE'">{{ ' - ' }} {{ new
                                                        Date(event.trdmcd).toLocaleTimeString('en-GB', {
                                                            hour: '2-digit',
                                                            minute: '2-digit', hour12: false
                                                        }) }} {{ ' to ' }} {{ new
                                                            Date(event.trdupd).toLocaleTimeString('en-GB', {
                                                                hour: '2-digit',
                                                                minute: '2-digit', hour12: false
                                                            }) }}</span>
                                                </div>
                                    ...
                                    <v-card v-show="event.trdsts != 'LEAVE'" color="grey lighten-4" min-width="250px"
                                        max-width="350px" max-height="500px" flat>
                                        ...
                                        <v-card-title primary-title>
                                            ...
                                            <v-btn
                                                v-show="event.validation == 'NOT APPROVE' && CurUserDetails.CNTMST.CNTMNN != 'AMM' && CurUserDetails.CNTMST.CNTMNN != 'MBB'"
                                                @click="approveItem(event)" :disabled="enableStart"
                                                color="primary">Approve</v-btn>
                                        </v-card-title>
                                        <v-card-actions>
                                        </v-card-actions>
                                    </v-card>
...
export default {
    ...
    watch: {
        selectedItemDepartment() {
            if (
                ...
                this.selectedItemDepartment.DPTNME
            ) {
    ...
    methods: {
        ...mapActions([
            "getITIMSTPerCNT", "getUserByDepartment", "updateForApproval"
        ]),
        ...
        approveItem(selectedEvent) {
            this.updateForApproval(selectedEvent.itimid).then(
                res => {
                    if (res.status == 200) {

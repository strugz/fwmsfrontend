...
export default {
  ...
  methods: {
    ...mapActions(["deleteSRCalendarAdditional","getServiceCalendar", "getThreadDetailsByTRDMID", "cancelMyItinerary"]),
    ...
    cancelItinerary(item) {
      this.cancelMyItinerary({ itimid: item.itimid }).then(res => {
        if (res.status == 200) {
    ...
    InsertCheckInStartService(event) {
      ...
      let data = this.CurUserDetails.CNTMST.CNTDPT.substring(0, 3) == "TSG" ? JSON.stringify({
        UserID: this.CurUserDetails.CNTMST.CNTMID,
